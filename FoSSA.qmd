...
-   $Z_{\alpha/2}$ is the critical value of the standard normal distribution at significance level $\alpha$,(e.g., 95% confidence corresponds to $Z_{\alpha/2} \approx 1.96$).
...
-   $Z_{\alpha/2}$ is the critical value for the desired confidence level (e.g., for 95% confidence, $Z_{\alpha/2} \approx 1.96$),
...
-   $Z_{\alpha/2}$ is the critical value for the desired significance level (e.g., for 95% confidence, $Z_{\alpha/2} \approx 1.96$),
...
- When the outcome and exposure and **binary** there are only `4 possible values` in the scatterplot @fig-Picture6 between SBP (1 ≥ 120 mmHg  and 0 < 120 mmHg)  and BMI (1 ≥ 25 kg/m2 and 
0 if < 25 kg/m2). So a `2x2 table` may be more appropriate.
...
A unit increase is given by:
< 18.5 (1, ref) vs 18.5-<25 kg/m2 (2), or
18.5-<25(2) vs 25-<30 kg/m2 (3), or
25-<30 (3) vs >= 30 kg/m2 (4)
...
OR=1.80 for 18.5-<25 versus < 18.5 kg/m2
OR=1.80 for 25-<30 versus 18.5-<25 kg/m2 
OR=1.80 for >= 30 versus 25-<30 kg/m2
...
The hazard function, denoted as $h(t)$, represents the instantaneous risk of an event occurring
after time $t$, given that the individual is event-free up to time $t$. In other word, What is the probability that the event will happen between $T$ and the next fraction of time. The formula is:
